...
public class ConverterGUI extends JFrame {
	...
	public ConverterGUI() {
		...
		setLayout(new GridLayout(10, 2, 0, 10));
		...
		// Add Convert Button
		JButton convertButton = new JButton("Convert (Enter)");
		add(convertButton);
		
		// Add Clear Button
		JButton clearButton = new JButton("Clear (Shift+Enter)");
		add(clearButton);
		...
		// Allows user to Convert again by Clearing the Fields
		// the user is not currently focused on
		clearOthersExcept(celsiusField);
		clearOthersExcept(fahrenheitField);
		clearOthersExcept(kelvinField);
		clearOthersExcept(centimetersField);
		clearOthersExcept(inchesField);
		clearOthersExcept(feetField);
		clearOthersExcept(f_FeetField);
		clearOthersExcept(f_InchField);
		clearOthersExcept(yardsField);
		clearOthersExcept(milesField);
	...
	// Allows the user to Convert again without needing to Clear it
	private void clearOthersExcept(JTextField focused) {
		JTextField[] fields = {celsiusField, fahrenheitField, kelvinField,
					centimetersField, inchesField, feetField, 
					f_FeetField, f_InchField,
					yardsField, milesField};
		
		for (int i = 0; i < fields.length; i++) {
			JTextField field = fields[i];
			
			// Edge case
			boolean isFeetAndInchesField = 
					(focused == f_FeetField && field == f_InchField || 
							focused == f_InchField && field == f_FeetField);
			
			if (field != focused && !isFeetAndInchesField) { field.setText(""); }
		}
	}
